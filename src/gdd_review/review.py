...
def build_review_crew() -> Crew:
    ...
    chief = Agent(
        role="主审",
        goal="汇总四维度对抗后的确认发现,输出决策者可直接使用的评审报告",
        backstory=(
    ...
    t_report = Task(
        description=(
            ...
            "# GDD 评审报告\n"
            "## 知识库状态\n"
            "（原样记录下面的门控结论;被跳过的维度说明原因并建议运行蒸馏）\n"
            f"{gate_summary}\n\n"
            "## 确认的发现\n"
            "### 错误(一致性)\n### 不足(深度)\n### 缺陷(反例命中)\n### 亮点\n"
            "每条含: 位置/严重度(blocker|major|minor)/证据/依据编号/修改建议\n"
            "## 已排除的指控(对抗质询中被推翻的,附理由)\n"
            "## 总评与优先级建议\n"
        ),
        expected_output="完整markdown评审报告",
        agent=chief,
